  ...
  const fetchSessionDetailsById = async () => {
    try {
      ...
      if(response.data && response.data.session) {
        setSessionData(response.data.session[0])
      }
    } catch(error) {
  ...
  return (
    <DashboardLayout>
      <RoleInfoHeader 
        ...
        lastUpdated={sessionData?.updatedAt ? moment(sessionData.updatedAt).format("DD MM YYYY"): ""}
       />
                    ...
                    <motion.div 
                      ...
                      {!isLoading && sessionData?.questions?.length == index+1 && (
                        <div className="flex items-center justify-center mt-5">
                          ...
                          disabled={isLoading || isUpdatedLoader} onClick={uploadMoreQuestions}>
                            {isUpdatedLoader ? (
                              <SpinnerLoader />
                            ): (
