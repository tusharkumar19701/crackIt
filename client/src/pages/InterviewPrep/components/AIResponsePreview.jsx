...
const AIResponsePreview = ({content}) => {
  ...
  return (
    <div className='max-w-4xl mx-auto'>
        <div className="text-[15px] text-slate-700 max-w-none ">
          <ReactMarkdown
            ...
            components={{
              ...
              th({children}) {
                return <th className='px-3 py-2 ext-left text-xs font-medium text-gray-500 uppercase tracking-wider '>{children}</th>
              },
              td({children}) {
                return <td className='px-3 py-2 whitespace-nowrap text-sm'>{children}</td>
              },
              hr() {
                return <hr className='my-6 broder-gray-200' />
              },
              img({src,alt}) {
  ...
  return (
    <div className='relative my-6 rounded-lg overflow-hidden bg-gray-50 border border-gray-200'>
      <div className="flex items-center justify-between px-4 py-2 bg-gray-100 border-b border-gray-200">
        ...
        <button
        onClick={copyCode}
        className="text-gray-500 hover:text-gray-700 focus:outline-none relative group" aria-label='Copy code'>
          {
            copied ? (
      ...
      language={language}
      style={oneLight}
      customStyle={{fontSize:15,font:'bold', margin:0, padding:'1rem',background:'transparent'}}
       >
        {code}
       </SyntaxHighlighter>
    </div>
